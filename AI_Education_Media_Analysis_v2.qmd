...
DATA_FILE_PATH <- "D:/LUKA/Academic/HKS/Clanci/AI u obrazovanju/dta.xlsx"
...
Educational discourse often intensifies around key calendar events. We examine whether media framing shifts during exam periods (državna matura, semester exams) and back-to-school seasons.

```{r}
#| label: calendar-setup

# Create academic calendar periods
clean_data <- clean_data %>%
  mutate(
    month_num = month(DATE),
    academic_period = case_when(
      month_num %in% c(6, 7) ~ "Matura/Exams (Jun-Jul)",
      month_num %in% c(8, 9) ~ "Back to School (Aug-Sep)",
      month_num %in% c(1, 2) ~ "Mid-Year Exams (Jan-Feb)",
      TRUE ~ "Regular Period"
    ),
    academic_period = factor(academic_period, levels = c(
      "Matura/Exams (Jun-Jul)", "Back to School (Aug-Sep)", 
      "Mid-Year Exams (Jan-Feb)", "Regular Period"
    ))
  )
```

### Frame Prevalence by Academic Period

```{r}
#| label: fig-calendar-frames
#| fig-cap: "Frame Distribution by Academic Calendar Period"
#| fig-height: 5

calendar_stats <- clean_data %>%
  ...
  summarise(
    ...
    opportunity = mean(frame_OPPORTUNITY_present, na.rm = TRUE) * 100,
    regulation = mean(frame_REGULATION_present, na.rm = TRUE) * 100,
    quality = mean(frame_QUALITY_present, na.rm = TRUE) * 100,
    mean_sentiment = mean(sentiment_score, na.rm = TRUE),
    .groups = "drop"
  )

calendar_stats %>%
  pivot_longer(c(threat, opportunity, regulation, quality), 
               names_to = "frame", values_to = "percentage") %>%
  mutate(frame = str_to_title(frame)) %>%
  ggplot(aes(x = academic_period, y = percentage, fill = frame)) +
  geom_col(position = "dodge", width = 0.7) +
  scale_fill_manual(values = c("Threat" = "#e41a1c", "Opportunity" = "#4daf4a", 
                                "Regulation" = "#377eb8", "Quality" = "#f39c12")) +
  labs(
    title = "Frame Usage by Academic Calendar Period",
    subtitle = "Comparing exam seasons vs. regular periods",
    x = NULL, y = "% of Articles", fill = "Frame"
  ) +
  theme(axis.text.x = element_text(angle = 15, hjust = 1))
```
...
Aggregating across years reveals seasonal patterns in frame usage. The threat frame typically peaks during matura season when plagiarism concerns dominate discourse.

```{r}
#| label: fig-monthly-frames
#| fig-cap: "Frame Prevalence by Month (Aggregated Across Years)"
#| fig-height: 5

monthly_by_month <- clean_data %>%
...
monthly_by_month %>%
  pivot_longer(c(threat, opportunity, regulation), 
               names_to = "frame", values_to = "percentage") %>%
  mutate(frame = str_to_title(frame)) %>%
  ggplot(aes(x = month_name, y = percentage, color = frame, group = frame)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 2.5) +
  scale_color_manual(values = c("Threat" = "#e41a1c", "Opportunity" = "#4daf4a", 
                                 "Regulation" = "#377eb8")) +
  labs(
    title = "Seasonal Frame Patterns",
    subtitle = "Monthly averages aggregated across study period",
    x = NULL, y = "% of Articles", color = "Frame"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

### Statistical Test: Calendar Effects

```{r}
#| label: calendar-stats

# ANOVA for threat frame by academic period
calendar_aov_threat <- aov(frame_THREAT_present ~ academic_period, data = clean_data)
calendar_aov_summary <- summary(calendar_aov_threat)

cat("ANOVA: Threat Frame by Academic Period\n")
print(calendar_aov_summary)

if (calendar_aov_summary[[1]]$`Pr(>F)`[1] < 0.05) {
  cat("\nPost-hoc Tukey HSD:\n")
  print(TukeyHSD(calendar_aov_threat))
}
```

```{r}
#| label: tbl-calendar
#| tbl-cap: "Summary Statistics by Academic Period"

calendar_table <- calendar_stats %>%
  mutate(
    `% Threat` = paste0(round(threat, 1), "%"),
    `% Opportunity` = paste0(round(opportunity, 1), "%"),
    `% Regulation` = paste0(round(regulation, 1), "%"),
    `Mean Sentiment` = round(mean_sentiment, 2)
  ) %>%
  select(
    `Academic Period` = academic_period,
    Articles = n,
    `% Threat`, `% Opportunity`, `% Regulation`, `Mean Sentiment`
  )

kable(calendar_table) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

---

## Political Orientation of Sources

Media outlets differ not only in format but in editorial orientation. We classify Croatian outlets along a conservative-liberal dimension to examine whether political leaning correlates with AI framing.

### Classification Schema

| Orientation | Outlets | Rationale |
|-------------|---------|-----------|
| **Conservative** | Večernji list, HRT | Traditional, establishment-aligned |
| **Liberal** | 24sata, Index, Jutarnji, Slobodna Dalmacija, Novi list, N1, T-portal, Net.hr | Progressive, commercially-oriented |

```{r}
#| label: political-setup

# Classify by political orientation
clean_data <- clean_data %>%
  mutate(
    political_orientation = case_when(
      str_detect(str_to_lower(FROM), "vecernji|hrt") ~ "Conservative",
      str_detect(str_to_lower(FROM), "24sata|index|jutarnji|slobodna|novi.?list|n1|tportal|net\\.hr") ~ "Liberal",
      TRUE ~ "Unclassified"
    )
  )

# Summary stats
political_n <- clean_data %>%
  count(political_orientation) %>%
  filter(political_orientation != "Unclassified")

cat("Sample sizes:\n")
print(political_n)
```

### Frame Comparison by Political Orientation

```{r}
#| label: fig-political-frames
#| fig-cap: "Frame Usage by Political Orientation of Outlet"
#| fig-height: 5

political_stats <- clean_data %>%
  filter(political_orientation != "Unclassified") %>%
  ...
  summarise(
    ...
    opportunity = mean(frame_OPPORTUNITY_present, na.rm = TRUE) * 100,
    regulation = mean(frame_REGULATION_present, na.rm = TRUE) * 100,
    disruption = mean(frame_DISRUPTION_present, na.rm = TRUE) * 100,
    replacement = mean(frame_REPLACEMENT_present, na.rm = TRUE) * 100,
    quality = mean(frame_QUALITY_present, na.rm = TRUE) * 100,
    mean_sentiment = mean(sentiment_score, na.rm = TRUE),
    .groups = "drop"
  )
...
political_stats %>%
  select(political_orientation, threat, opportunity, regulation, disruption, replacement) %>%
  pivot_longer(-political_orientation, names_to = "frame", values_to = "percentage") %>%
  mutate(frame = str_to_upper(frame)) %>%
  ggplot(aes(x = frame, y = percentage, fill = political_orientation)) +
  geom_col(position = "dodge", width = 0.7) +
  scale_fill_manual(values = c("Conservative" = "#c0392b", "Liberal" = "#2980b9")) +
  labs(
    title = "Frame Prevalence by Political Orientation",
    subtitle = "Conservative vs. Liberal outlets",
    x = NULL, y = "% of Articles", fill = "Orientation"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
#| label: fig-political-sentiment
#| fig-cap: "Sentiment Distribution by Political Orientation"
#| fig-height: 5

clean_data %>%
  filter(political_orientation != "Unclassified") %>%
  ggplot(aes(x = political_orientation, y = sentiment_score, fill = political_orientation)) +
  geom_boxplot(alpha = 0.7, outlier.alpha = 0.3) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  scale_fill_manual(values = c("Conservative" = "#c0392b", "Liberal" = "#2980b9")) +
  labs(
    title = "Sentiment Distribution by Political Orientation",
    subtitle = "Box plots showing median, IQR, and outliers",
    x = NULL, y = "Sentiment Score"
  ) +
  theme(legend.position = "none")
```

### Statistical Test: Political Orientation

```{r}
#| label: political-stats

# T-test for sentiment differences
political_ttest <- t.test(
  sentiment_score ~ political_orientation, 
  data = clean_data %>% filter(political_orientation != "Unclassified")
)

cat("Independent Samples t-test: Sentiment by Political Orientation\n")
cat("  t =", round(political_ttest$statistic, 3), "\n")
cat("  df =", round(political_ttest$parameter, 1), "\n")
cat("  p-value =", format(political_ttest$p.value, digits = 4), "\n")
cat("  Mean (Conservative):", round(political_ttest$estimate[1], 3), "\n")
cat("  Mean (Liberal):", round(political_ttest$estimate[2], 3), "\n")

# Chi-square for frame-orientation association
political_frame_table <- table(
  clean_data$dominant_frame[clean_data$political_orientation != "Unclassified"],
  clean_data$political_orientation[clean_data$political_orientation != "Unclassified"]
)
political_chisq <- chisq.test(political_frame_table)

cat("\nChi-Square Test: Dominant Frame vs. Political Orientation\n")
cat("  χ² =", round(political_chisq$statistic, 2), "\n")
cat("  df =", political_chisq$parameter, "\n")
cat("  p-value =", format(political_chisq$p.value, scientific = TRUE), "\n")
```

```{r}
#| label: tbl-political
#| tbl-cap: "Summary Statistics by Political Orientation"

political_table <- political_stats %>%
  mutate(
    `% Threat` = paste0(round(threat, 1), "%"),
    `% Opportunity` = paste0(round(opportunity, 1), "%"),
    `% Regulation` = paste0(round(regulation, 1), "%"),
    `% Replacement` = paste0(round(replacement, 1), "%"),
    `Mean Sentiment` = round(mean_sentiment, 2)
  ) %>%
  select(
    Orientation = political_orientation,
    Articles = n,
    `% Threat`, `% Opportunity`, `% Regulation`, `% Replacement`, `Mean Sentiment`
  )

kable(political_table) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
While frequency counts reveal *who* is mentioned, they do not capture *how* actors are discussed. This section examines the linguistic context surrounding key actors through co-occurrence analysis and frame associations.

### Methodological Approach

For each actor category, we:

1. Extract text windows (±50 characters) around actor mentions
2. Identify frequently co-occurring terms
3. Examine frame associations when each actor is primary

This reveals the semantic fields and evaluative contexts surrounding different stakeholders.

### Co-occurring Terms by Actor

```{r}
#| label: context-extraction

# Function to extract context windows
extract_context_words <- function(texts, actor_terms, window = 50) {
  all_words <- c()
  
  for (text in texts) {
    if (is.na(text)) next
    text_lower <- str_to_lower(text)
    pattern <- paste0("\\b(", paste(actor_terms, collapse = "|"), ")\\b")
    
    matches <- str_locate_all(text_lower, pattern)[[1]]
    if (nrow(matches) == 0) next
    
    for (i in seq_len(nrow(matches))) {
      start <- max(1, matches[i, 1] - window)
      end <- min(nchar(text), matches[i, 2] + window)
      context <- substr(text_lower, start, end)
      words <- unlist(str_extract_all(context, "\\b[a-zčćžšđ]{4,}\\b"))
      all_words <- c(all_words, words)
    }
  }
  
  # Remove actor terms and stopwords
  stopwords_hr <- c("koji", "koja", "koje", "biti", "jest", "nisu", "mogu", "može", 
                    "tako", "više", "manje", "samo", "također", "prema", "kroz",
                    "ovaj", "ova", "ovo", "svoj", "svoja", "svoje", "nego", "kada",
                    "sada", "nakon", "prije", "zato", "stoga", "međutim", "ipak")
  all_words <- all_words[!all_words %in% c(actor_terms, stopwords_hr)]
  
  word_counts <- sort(table(all_words), decreasing = TRUE)
  data.frame(
    word = names(word_counts),
    n = as.integer(word_counts)
  ) %>% head(15)
}

# Define actor terms
student_terms <- c("student", "studenti", "studentica", "učenik", "učenici", "učenica", "đak", "maturant")
teacher_terms <- c("učitelj", "učitelji", "učiteljica", "nastavnik", "nastavnici", "profesor", "profesori", "profesorica")
institution_terms <- c("škola", "škole", "fakultet", "sveučilište", "ministarstvo", "gimnazija", "osnovna")

# Extract co-occurring words (using sample for efficiency)
set.seed(42)
sample_texts <- sample(clean_data$FULL_TEXT, min(1000, nrow(clean_data)))

student_context <- extract_context_words(sample_texts, student_terms)
teacher_context <- extract_context_words(sample_texts, teacher_terms)
institution_context <- extract_context_words(sample_texts, institution_terms)
```

```{r}
#| label: fig-student-context
#| fig-cap: "Most Frequent Terms Co-occurring with Student Mentions"
#| fig-height: 4

if (nrow(student_context) > 0) {
  ggplot(student_context, aes(x = reorder(word, n), y = n)) +
    geom_col(fill = "#3498db", alpha = 0.8) +
    coord_flip() +
    labs(
      title = "Student Context: Co-occurring Terms",
      subtitle = "Words appearing within ±50 characters of student mentions",
      x = NULL, y = "Frequency"
    )
}
```
...
```{r}
#| label: fig-teacher-context
#| fig-cap: "Most Frequent Terms Co-occurring with Teacher Mentions"
#| fig-height: 4

if (nrow(teacher_context) > 0) {
  ggplot(teacher_context, aes(x = reorder(word, n), y = n)) +
    geom_col(fill = "#e67e22", alpha = 0.8) +
    coord_flip() +
    labs(
      title = "Teacher Context: Co-occurring Terms",
      subtitle = "Words appearing within ±50 characters of teacher mentions",
      x = NULL, y = "Frequency"
    )
}
```
...
```{r}
#| label: fig-institution-context
#| fig-cap: "Most Frequent Terms Co-occurring with Institution Mentions"
#| fig-height: 4

if (nrow(institution_context) > 0) {
  ggplot(institution_context, aes(x = reorder(word, n), y = n)) +
    geom_col(fill = "#9b59b6", alpha = 0.8) +
    coord_flip() +
    labs(
      title = "Institution Context: Co-occurring Terms",
      subtitle = "Words appearing within ±50 characters of institution mentions",
      x = NULL, y = "Frequency"
    )
}
```

### Frame Associations by Primary Actor

When an actor dominates an article (highest mention count), which frames tend to co-occur? This reveals systematic differences in how stakeholders are portrayed.

```{r}
#| label: fig-actor-frame-detail
#| fig-cap: "Detailed Frame Associations by Primary Actor"
#| fig-height: 6

actor_frame_detail <- clean_data %>%
  filter(primary_actor != "NONE") %>%
  group_by(primary_actor) %>%
  summarise(
    n = n(),
    ...
    replacement = mean(frame_REPLACEMENT_present, na.rm = TRUE) * 100,
    quality = mean(frame_QUALITY_present, na.rm = TRUE) * 100,
    competence = mean(frame_COMPETENCE_present, na.rm = TRUE) * 100,
    .groups = "drop"
  ) %>%
  filter(n >= 30)  # Minimum sample size

actor_frame_detail %>%
  pivot_longer(c(threat, opportunity, regulation, disruption, replacement, quality, competence),
               names_to = "frame", values_to = "percentage") %>%
  mutate(
    frame = str_to_upper(frame),
    primary_actor = str_replace_all(primary_actor, "_", " ")
  ) %>%
  ggplot(aes(x = frame, y = primary_actor, fill = percentage)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = round(percentage, 1)), size = 3) +
  scale_fill_viridis_c(option = "plasma", direction = -1) +
  labs(
    title = "Frame-Actor Association Matrix",
    subtitle = "% of articles containing each frame when actor is primary",
...
```{r}
#| label: tbl-actor-context
#| tbl-cap: "Frame Prevalence by Primary Actor"

actor_context_table <- actor_frame_detail %>%
  mutate(
    Actor = str_replace_all(primary_actor, "_", " "),
    `% Threat` = round(threat, 1),
    `% Opportunity` = round(opportunity, 1),
    `% Regulation` = round(regulation, 1),
    `% Quality` = round(quality, 1)
  ) %>%
  select(Actor, N = n, `% Threat`, `% Opportunity`, `% Regulation`, `% Quality`) %>%
  arrange(desc(N))

kable(actor_context_table) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```

### Interpretation

The context analysis reveals distinct discursive positions:

- **Students** appear predominantly in contexts related to academic integrity (plagiarism, cheating, exams), associated with elevated threat framing
- **Teachers** are discussed in relation to pedagogical adaptation and professional development, with more balanced framing
- **Institutions** feature prominently in regulatory and policy contexts
- **Tech companies** dominate opportunity framing, typically in product announcement or feature coverage

These patterns suggest media discourse constructs students primarily as *potential rule-breakers* rather than *learners*, while positioning teachers as *adapters* navigating technological change.
